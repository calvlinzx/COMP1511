...
int isYear(struct date *start_day, struct date *finish_day, struct pod *n);
int isDay(struct date *start_day, struct date *finish_day, struct pod *n);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    for (struct pod *n = first_pod; n != NULL; n = n->next) {
        if (isYear(start_day, finish_day, n) == 0) { 
            if (start_day->month < finish_day->month) {
                if (n->when->month == start_day->month) {
                    if (n->when->day >= start_day->day) {
                        printf("%02d/%02d/%02d %2d %s\n",n->when->day, n->when->month, n->when->year, n->how_many, n->species);
                    }        
                }
                else if (n->when->month == finish_day->month) {
                    if (n->when->day <= finish_day->day) {
                        printf("%02d/%02d/%02d %2d %s\n",n->when->day, n->when->month, n->when->year, n->how_many, n->species);
                    }
                }
                else if (n->when->month > start_day->month && n->when->month < finish_day->month){
                        printf("%02d/%02d/%02d %2d %s\n",n->when->day, n->when->month, n->when->year, n->how_many, n->species);
                }    
            }
            else if (start_day->month && n->when->month == finish_day->month) {
                if (isDay(start_day, finish_day, n) == -1) {
                    printf("%02d/%02d/%02d %2d %s\n",n->when->day, n->when->month, n->when->year, n->how_many, n->species);
                }
            } 
        }
        else if (isYear(start_day, finish_day, n) == 1) {
            if (n->when->month == start_day->month) {         
                if (isDay(start_day, finish_day, n) == 0) {
                    printf("%02d/%02d/%02d %2d %s\n",n->when->day, n->when->month, n->when->year, n->how_many, n->species);
                }
        }
            else if (n->when->month > start_day->month) {
                printf("%02d/%02d/%02d %2d %s\n",n->when->day, n->when->month, n->when->year, n->how_many, n->species);
            }
        }
        else if (isYear(start_day, finish_day, n) == -1) {
            if (n->when->month == finish_day->month) {         
                if (isDay(start_day, finish_day, n) == 1) {
                    printf("%02d/%02d/%02d %2d %s\n",n->when->day, n->when->month, n->when->year, n->how_many, n->species);
                }
            }
...
int isYear(struct date *start_day, struct date *finish_day, struct pod *n) {
    if (n->when->year == start_day->year && n->when->year == finish_day->year) {
        return 0;
    }
    else if (n->when->year == start_day->year && n->when->year < finish_day->year) {
        return 1;
    }  
    else if (n->when->year > start_day->year && n->when->year == finish_day->year) {
        return -1;
...
int isDay(struct date *start_day, struct date *finish_day, struct pod *n) {
    ...
    else if (n->when->day <= finish_day->day) {
        return 1;    
    }
    else {
        return 2;
