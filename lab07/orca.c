// To complete the count_orca_sightings function
// Starting code for COMP1511 lab exercises
// Zexin Lin (z5217017)
// Modified on 04/09/18
...
int count_orca_sightings(int n_sightings, struct pod sightings[n_sightings]) {
    ...
    while (i < n_sightings) {
        if (!strcmp("Orca", sightings[i].species)) {
            n_orca++;
        }
        i++;
    }
    
    return n_orca; // CHANGE ME
}
...
int read_sightings_file(char filename[], int len, struct pod sightings[len]) {
    ...
    int n_sightings = 0;
    while (n_sightings < len && read_sighting(f, &sightings[n_sightings]) == 1) {
