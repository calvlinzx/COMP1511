...
void species_count(char species[], int n_sightings, struct pod sightings[n_sightings], int *n_pods, int *n_whales) {
    ...
    while (i < n_sightings) {
        if (!strcmp(species, sightings[i].species)) {
            pods_count++;
            whales_count = whales_count + sightings[i].how_many;
        }
        i++;
    }
    
    *n_pods = pods_count; // CHANGE ME
    *n_whales = whales_count; // CHANGE ME
}
